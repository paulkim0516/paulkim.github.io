 ...
 * @returns {THREE.Vector3} The closest point on the given curve.
 */
function curveClosestPointA(point, curve, tolerance) {
    let curveGeo = curve.geometry.userData.curve;
    let minDistance = Infinity;
    let t = 0;
    ...
    return curveGeo.getPointAt(tSampled[idx]).add(curve.position);
}
        ...
        row.children.forEach(circle => {
            ...
            curves.children.forEach(curve => {
                ...
                if (!curveUUID || curve.uuid == curveUUID || !(curveUUID in circle.userData)) {
                    ...
                    const calc = curveClosestPointA(circleCenter, curve, tolerance)
                    console.log(calc);
                    distance = circleCenter.distanceTo(calc);
